...
import { onRequest } from './transcribe';
...
describe('POST /api/ai/transcribe', () => {
  ...
  test('given empty body > returns 400 (must provide audioBase64 or audioUrl)', async () => {
    vi.mocked(getSessionFromRequest).mockResolvedValue(okSession);
    const env = buildEnv();
  ...
  test('given audioUrl + starter plan > returns 200 with segments', async () => {
    vi.mocked(getSessionFromRequest).mockResolvedValue(okSession);
    const env = buildEnv();
    const res = await onRequest({
      env,
      request: buildRequest({ audioUrl: 'https://example.com/audio.mp3' }),
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
      ok: boolean;
      data: { fullText: string; segments: { startMs: number }[] };
      provider: string;
    };
    expect(body.ok).toBe(true);
    expect(typeof body.data.fullText).toBe('string');
